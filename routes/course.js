...
router.get("/findByName/:name", (req, res) => {
  ...
  Course.find({ title: name })
    ...
    .catch((err) => {
      res.status(500).send("Cannot find the course.");
    });
});
...
router.patch("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course orweb admin can edit this course.",
    });
  }
});
...
router.delete("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course orweb admin can delete this course.",
    });
  }
});
